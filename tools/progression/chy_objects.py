requirements = {
#Map          ItemLocation      Requires            VanillaItem
# Gate District
'MAC_00':    {'ShopItemA':      [],                 # 'FrightJar',
              'ShopItemB':      [],                 # 'SleepySheep',
              'ShopItemC':      [],                 # 'POWBlock',
              'ShopItemD':      [],                 # 'FireFlower',
              'ShopItemE':      [],                 # 'HoneySyrup',
              'ShopItemF':      [],                 # 'Mushroom',
              'HiddenPanel':    [("SuperBoots",),
                                 ("UltraHammer",)], # 'StarPiece'
              'ItemA':          [("Sushie",)],      # 'StarPiece'
              },
# Residental District
'MAC_04':    {'ShopItemA':      [],                 # 'StoneCap',
              'ShopItemB':      [],                 # 'DizzyDial',
              'ShopItemC':      [],                 # 'ThunderRage',
              'ShopItemD':      [],                 # 'TastyTonic',
              'ShopItemE':      [],                 # 'VoltShroom',
              'ShopItemF':      [],                 # 'SuperShroom'
              },
# Star Haven
'HOS_03':    {'ShopItemA':      [],                 # 'StopWatch',
              'ShopItemB':      [],                 # 'ShootingStar',
              'ShopItemC':      [],                 # 'SuperSoda',
              'ShopItemD':      [],                 # 'MapleSyrup',
              'ShopItemE':      [],                 # 'LifeShroom',
              'ShopItemF':      [],                 # 'SuperShroom'
              },
# Koopa Village 1
'NOK_01':    {'ShopItemA':      [],                 # 'DizzyDial',
              'ShopItemB':      [],                 # 'POWBlock',
              'ShopItemC':      [],                 # 'FireFlower',
              'ShopItemD':      [],                 # 'HoneySyrup',
              'ShopItemE':      [],                 # 'VoltShroom',
              'ShopItemF':      [],                 # 'Mushroom',
              'HiddenPanel':    [("SuperBoots",),
                                 ("UltraHammer",)], # 'StarPiece',
              'Bush6A_Drop1A':  [("p_KKF15",)],     # 'Glasses',
              'Bush7A_Drop1A':  [("p_KKF06",)],     # 'EmptyWallet'
              },
# Outpost 1
'DRO_01':    {'ShopItemA':      [],                 # 'ThunderBolt',
              'ShopItemB':      [],                 # 'DustyHammer',
              'ShopItemC':      [],                 # 'HoneySyrup',
              'ShopItemD':      [],                 # 'DriedShroom',
              'ShopItemE':      [],                 # 'DriedPasta',
              'ShopItemF':      [],                 # 'Mushroom',
              'GiftA':          [],                 # 'Melody',
              'GiftB':          [],                 # 'RedJar'
              },
# Shiver City Center
'SAM_02':    {'ShopItemA':      [],                 # 'DizzyDial',
              'ShopItemB':      [],                 # 'ShootingStar',
              'ShopItemC':      [],                 # 'SnowmanDoll',
              'ShopItemD':      [],                 # 'MapleSyrup',
              'ShopItemE':      [],                 # 'LifeShroom',
              'ShopItemF':      [],                 # 'SuperShroom',
              'ItemA':          [],                 # 'IcedPotato',
              'ItemB':          [],                 # 'UltraShroom',
              'ItemC':          [],                 # 'Mushroom',
              'ItemD':          [],                 # 'Mushroom',
              'ItemE':          [],                 # 'Mushroom',
              'ItemF':          [],                 # 'Mushroom'
              },
# Basement
'OBK_03':    {'ShopItemA':      [],                 # 'Mystery',
              'ShopItemB':      [],                 # 'StopWatch',
              'ShopItemC':      [],                 # 'SnowmanDoll',
              'ShopItemD':      [],                 # 'MapleSyrup',
              'ShopItemE':      [],                 # 'LifeShroom',
              'ShopItemF':      [],                 # 'SuperShroom',
              'CrateA':         [],                 # 'SuperShroom'
              },
# Village Buildings
'JAN_03':    {'ShopItemA':      [],                 # 'SnowmanDoll',
              'ShopItemB':      [],                 # 'ThunderRage',
              'ShopItemC':      [],                 # 'FireFlower',
              'ShopItemD':      [],                 # 'TastyTonic',
              'ShopItemE':      [],                 # 'HoneySyrup',
              'ShopItemF':      [],                 # 'SuperShroom',
              'GiftA':          [],                 # 'MagicalSeed4'
              },
# Item Shop
'KPA_96':    {'ShopItemA':      [],                 # 'Mystery',
              'ShopItemB':      [],                 # 'DizzyDial',
              'ShopItemC':      [],                 # 'POWBlock',
              'ShopItemD':      [],                 # 'ThunderRage',
              'ShopItemE':      [],                 # 'MapleSyrup',
              'ShopItemF':      [],                 # 'SuperShroom'
              },
# Forest Clearing
'KMR_00':    {'HiddenPanel':    [],                 # 'StarPiece'
              },
# Bottom of the Cliff
'KMR_03':    {'HiddenPanel':    [],                 # 'StarPiece',
              'HiddenYBlockA':  [],                 # 'RepelGel',
              'YBlockA':        [],                 # 'Coin',
              'ItemA':          [],                 # 'Coin',
              'ItemB':          [],                 # 'Coin',
              'ItemC':          [],                 # 'Coin',
              'ItemD':          [],                 # 'Coin',
              'ItemE':          [],                 # 'FireFlower',
              'ItemF':          [],                 # 'Mushroom'
              },
# Goomba King's Castle
'KMR_11':    {'HiddenPanel':    [],                 # 'StarPiece',
              'YBlockA':        [],                 # 'SuperShroom',
              'Tree1_Drop1A':   [],                 # 'StarPiece'
              },
# Southern District
'MAC_02':    {'HiddenPanel':    [],                 # 'StarPiece',
              'ItemA':          [],                 # 'OddKey',
              'GiftA':          [],                 # 'MagicalSeed1'
              },
# Station District
'MAC_03':    {'HiddenPanel':    [],                 # 'StarPiece',
              'GiftA':          [],                 # 'Letter21',
              'GiftB':          [],                 # 'Letter23'
              },
# Port District
'MAC_05':    {'HiddenPanel':    [],                 # 'StarPiece',
              'GiftA':          [],                 # 'Lyrics'
              },
# Shooting Star Path
'HOS_00':    {'HiddenPanel':    [],                 # 'StarPiece'
              },
# Shooting Star Summit
'HOS_01':    {'HiddenPanel':    [],                 # 'StarPiece',
              'ItemA':          [],                 # 'StarPiece'
              },
# Merluvlee's House
'HOS_06':    {'HiddenPanel':    [],                 # 'StarPiece',
              'GiftA':          [],                 # 'Autograph2'
              },
# Pleasant Crossroads
'NOK_13':    {'HiddenPanel':    [],                 # 'StarPiece',
              'RBlockA':        [],                 # 'AttackFXB',
              'ItemA':          [],                 # 'HoneySyrup'
              },
# Path to Fortress 1
'NOK_14':    {'HiddenPanel':    [],                 # 'StarPiece',
              'ItemA':          [],                 # 'Coin',
              'ItemB':          [],                 # 'Coin',
              'ItemC':          [],                 # 'Coin',
              'ItemD':          [],                 # 'Coin',
              'ItemE':          [],                 # 'Coin',
              'ItemF':          [],                 # 'ThunderBolt',
              'HiddenYBlockA':  [],                 # 'FireFlower'
              },
# Mt Rugged 2
'IWA_01':    {'HiddenPanel':    [],                 # 'StarPiece',
              'ItemA':          [],                 # 'QuakeHammer',
              'ItemB':          [],                 # 'Letter01'
              },
# Outpost 2
'DRO_02':    {'HiddenPanel':    [],                 # 'StarPiece',
              'ItemA':          [],                 # 'Letter08',
              'GiftA':          [],                 # 'CrystalBall',
              'GiftB':          [],                 # 'PulseStone'
              },
# Center (Tweester C)
'SBK_33':    {'HiddenPanel':    [],                 # 'StarPiece'
              },
# Exit to Gusty Gulch
'MIM_12':    {'HiddenPanel':    [],                 # 'StarPiece'
              },
# Foyer
'OBK_01':    {'HiddenPanel':    [],                 # 'StarPiece',
              'GiftA':          [],                 # 'OldPhoto'
              },
# Basement Stairs
'OBK_02':    {'HiddenPanel':    [],                 # 'StarPiece'
              },
# Super Boots Room
'OBK_04':    {'HiddenPanel':    [],                 # 'StarPiece',
              'CrateA':         [],                 # 'MapleSyrup'
              },
# Record Room
'OBK_08':    {'HiddenPanel':    [],                 # 'StarPiece',
              'ItemA':          [],                 # 'Record'
              },
# BLU Station
'OMO_03':    {'HiddenPanel':    [],                 # 'StarPiece',
              'HiddenYBlockA':  [],                 # 'StoneCap'
              },
# PNK Station
'OMO_06':    {'HiddenPanel':    [],                 # 'StarPiece',
              'ChestA':         [],                 # 'Mailbag',
              'HiddenYBlockA':  [],                 # 'ThunderRage'
              },
# GRN Station
'OMO_08':    {'HiddenPanel':    [],                 # 'StarPiece',
              'HiddenYBlockA':  [],                 # 'FireFlower'
              },
# RED Station
'OMO_10':    {'HiddenPanel':    [],                 # 'StarPiece',
              'HiddenYBlockA':  [],                 # 'SuperShroom'
              },
# Village Cove
'JAN_02':    {'HiddenPanel':    [],                 # 'StarPiece',
              'GiftA':          [],                 # 'JadeRaven'
              },
# Deep Jungle 4 (Ambush)
'JAN_15':    {'HiddenPanel':    [],                 # 'StarPiece'
              },
# Zipline Cavern
'KZN_09':    {'HiddenPanel':    [],                 # 'StarPiece'
              },
# Boss Antechamber
'KZN_18':    {'HiddenPanel':    [],                 # 'StarPiece'
              },
# (East) Petunia's Field
'FLO_03':    {'HiddenPanel':    [],                 # 'StarPiece',
              'GiftA':          [],                 # 'MagicalBean'
              },
# (SE) Water Level Room
'FLO_24':    {'HiddenPanel':    [],                 # 'StarPiece',
              'YBlockA':        [],                 # 'DizzyDial',
              'HiddenYBlockA':  [],                 # 'MapleSyrup'
              },
# (SW) Path to Crystal Tree
'FLO_25':    {'HiddenPanel':    [],                 # 'StarPiece'
              },
# Shiver City Mayor Area
'SAM_01':    {'HiddenPanel':    [],                 # 'StarPiece',
              'GiftA':          [],                 # 'Bucket',
              'ChestA':         [],                 # 'AttackFXE'
              },
# Shiver Snowfield
'SAM_04':    {'HiddenPanel':    [],                 # 'StarPiece',
              'ItemA':          [],                 # 'Letter05',
              'ItemB':          [],                 # 'RepelGel'
              },
# Huge Statue Room
'PRA_21':    {'HiddenPanel':    [],                 # 'StarPiece',
              'YBlockA':        [],                 # 'MapleSyrup'
              },
# Small Statue Room
'PRA_22':    {'HiddenPanel':    [],                 # 'StarPiece',
              'HiddenYBlockA':  [],                 # 'JamminJelly'
              },
# Goomba Village
'KMR_02':    {'GiftA':          [],                 # 'Tape',
              'ItemA':          [],                 # 'ShootingStar',
              'Tree1_Drop1A':   [],                 # 'Goomnut'
              },
# Jr. Troopa's Playground
'KMR_04':    {'ItemA':          [],                 # 'Dolly'
              },
# Behind the Village
'KMR_05':    {'ItemA':          [],                 # 'StarPiece',
              'ItemB':          [],                 # 'Coin'
              },
# Goomba Road 2
'KMR_06':    {'RBlockA':        [],                 # 'CloseCall',
              'ItemA':          [],                 # 'Mushroom'
              },
# Goomba Road 1
'KMR_09':    {'YBlockA':        [],                 # 'Coin',
              'YBlockB':        [],                 # 'Coin'
              },
# Toad Town Entrance
'KMR_10':    {'ChestA':         [],                 # 'HammerThrow',
              'YBlockA':        [],                 # 'SleepySheep'
              },
# Plaza District
'MAC_01':    {'ItemA':          [],                 # 'QuickChange',
              'Tree1_Drop1A':   [],                 # 'StarPiece'
              },
# Blooper Boss 1 (B1)
'TIK_02':    {'ChestA':         [],                 # 'ShrinkStomp'
              },
# Short Elevator Room (B1)
'TIK_03':    {'YBlockA':        [],                 # 'SnowmanDoll',
              'YBlockB':        [],                 # 'Coin',
              'YBlockC':        [],                 # 'Coin'
              },
# Spring Room (B2)
'TIK_05':    {'ChestA':         [],                 # 'PowerSmash1'
              },
# Elevator Attic Room (B2)
'TIK_07':    {'ItemA':          [],                 # 'StarPiece'
              },
# Block Puzzle Room (B2)
'TIK_10':    {'HiddenYBlockA':  [],                 # 'Coin',
              'HiddenYBlockB':  [],                 # 'Coin',
              'HiddenYBlockC':  [],                 # 'Coin'
              },
# Hall to Blooper 1 (B1)
'TIK_18':    {'HiddenYBlockA':  [],                 # 'SuperShroom'
              },
# Room with Spikes (B2)
'TIK_20':    {'YBlockA':        [],                 #'ShootingStar'
              },
# Hidden Blocks Room (B2)
'TIK_21':    {'YBlockA':        [],                 # 'Coin',
              'HiddenYBlockA':  [],                 # 'Coin',
              'HiddenYBlockB':  [],                 # 'Coin',
              'HiddenYBlockC':  [],                 # 'Coin',
              'HiddenYBlockD':  [],                 # 'Coin'
              },
# Windy Path (B3)
'TIK_23':    {'HiddenYBlockA':  [],                 # 'MapleSyrup',
              'HiddenYBlockB':  [],                 # 'StopWatch',
              'HiddenYBlockC':  [],                 # 'VoltShroom',
              'YBlockA':        [],                 # 'Coin'
              },
# Hall to Ultra Boots (B3)
'TIK_24':    {'HiddenYBlockA':  [],                 # 'LifeShroom',
              'YBlockA':        [],                 # 'Coin',
              'YBlockB':        [],                 # 'Coin'
              },
# Library (2F)
'KKJ_16':    {'ItemA':          [],                 # 'LifeShroom',
              'ItemB':          [],                 # 'PowerRush'
              },
# Storeroom (2F)
'KKJ_17':    {'ItemA':          [],                 # 'DeepFocus1'
              },
# Guest Room (1F)
'KKJ_20':    {'ChestA':         [],                 #'LastStand'
              },
# Koopa Village 2
'NOK_02':    {'ItemA':          [],                 # 'StarPiece',
              'GiftA':          [],                 # 'KoopaLegends',
              'GiftB':          [],                 # 'SilverCredit',
              'GiftC':          [],                 # 'GoldCredit'
              },
# Behind Koopa Village
'NOK_03':    {'ItemA':          [],                 # 'HPPlusB'
              },
# Fuzzy Forest
'NOK_04':    {'GiftA':          [],                 # 'KoopersShell'
              },
# Pleasant Path Entry
'NOK_11':    {'YBlockA':        [],                 # 'Coin',
              'RBlockA':        [],                 # 'DizzyAttack',
              'YBlockB':        [],                 # 'FrightJar'
              },
# Pleasant Path Bridge
'NOK_12':    {'YBlockA':        [],                 # 'POWBlock',
              'ItemA':          [],                 # 'StarPiece',
              'ItemB':          [],                 # 'SleepySheep'
              },
# Path to Fortress 2
'NOK_15':    {'Tree1_Drop1A':   [],                 # 'StarPiece'
              },
# Fortress Exterior
'TRD_00':    {'ChestA':         [],                 # 'Refund',
              'ChestB':         [],                 # 'FPPlusB'
              },
# Left Tower
'TRD_01':    {'ItemA':          [],                 # 'SmashCharge',
              'ItemB':          [],                 # 'FortressKey'
              },
# Central Hall
'TRD_03':    {'ItemA':          [],                 # 'FortressKey',
              'ItemB':          [],                 # 'FortressKey',
              'ItemC':          [],                 # 'PowerBounce'
              },
# Dungeon Fire Room
'TRD_08':    {'ItemA':          [],                 # 'FortressKey'
              },
# Battlement
'TRD_09':    {'YBlockA':        [],                 # 'MapleSyrup'
              },
# Mt Rugged 1
'IWA_00':    {'ItemA':          [],                 # 'Coin',
              'ItemB':          [],                 # 'Coin',
              'ItemC':          [],                 # 'Coin',
              'YBlockA':        [],                 # 'SleepySheep',
              'ItemD':          [],                 # 'WhackasBump'
              },
# Mt Rugged 3
'IWA_02':    {'ItemA':          [],                 # 'StarPiece',
              'GiftA':          [],                 # 'MagicalSeed2'
              },
# Mt Rugged 4
'IWA_03':    {'ChestA':         [],                 # 'DamageDodgeB',
              'YBlockA':        [],                 # 'Coin',
              'YBlockB':        [],                 # 'Mushroom',
              'YBlockC':        [],                 # 'HoneySyrup',
              'ItemA':          [],                 # 'Letter25',
              'ItemB':          [],                 # 'StarPiece',
              'ItemC':          [],                 # 'Coin',
              'ItemD':          [],                 # 'Coin',
              'ItemE':          [],                 # 'Coin',
              'ItemF':          [],                 # 'Coin',
              'ItemG':          [],                 # 'Coin',
              'ItemH':          [],                 # 'Coin',
              'ItemI':          [],                 # 'Coin',
              'ItemJ':          [],                 # 'Coin'
              },
# Suspension Bridge
'IWA_04':    {'ItemA':          [],                 # 'Letter10'
              },
# Train Station
'IWA_10':    {'GiftA':          [],                 # 'Letter01',
              'GiftB':          [],                 # 'Letter25',
              'GiftC':          [],                 # 'Letter10'
              },
# N3W3
'SBK_00':    {'YBlockA':        [],                 # 'FrightJar',
              'YBlockB':        [],                 # 'Coin'
              },
# N3E2 Pokey Army
'SBK_05':    {'ItemA':          [],                 # 'FireFlower'
              },
# N2W3
'SBK_10':    {'HiddenYBlockA':  [],                 # 'ThunderRage'
              },
# N2E1 (Tweester A)
'SBK_14':    {'YBlockA':        [],                 # 'Coin',
              'YBlockB':        [],                 # 'HoneySyrup'
              },
# N1W3 Special Block
'SBK_20':    {'YBlockA':        [],                 # 'Mushroom',
              'YBlockB':        [],                 # 'SuperShroom',
              'YBlockC':        [],                 # 'UltraShroom'
              },
# N1W1
'SBK_22':    {'YBlockA':        [],                 # 'Coin',
              'YBlockB':        [],                 # 'Coin',
              'YBlockC':        [],                 # 'Coin',
              'YBlockD':        [],                 # 'Coin',
              'YBlockE':        [],                 # 'FireFlower'
              },
# N1E1 Palm Trio
'SBK_24':    {'HiddenRBlockA':  [],                 # 'RunawayPay'
              },
# W3 Kolorado's Camp
'SBK_30':    {'Tree2_Drop1A':   [],                 # 'Letter02'
              },
# E3 Outside Outpost
'SBK_36':    {'Tree9_Drop1A':   [],                 # 'Letter03'
              },
# S1
'SBK_43':    {'YBlockA':        [],                 # 'Coin'
              },
# S1E2 Small Bluffs
'SBK_45':    {'ItemA':          [],                 # 'StopWatch',
              'ItemB':          [],                 # 'SpinAttack'
              },
#S1E3 North of Oasis
'SBK_46':    {'YBlockA':        [],                 # 'Coin',
              'HiddenYBlockA':  [],                 # 'LifeShroom'
              },
# S2E2 West of Oasis
'SBK_55':    {'ItemA':          [],                 # 'TastyTonic'
              },
# S2E3 Oasis
'SBK_56':    {'Tree1_Drop1A':   [],                 # 'Lemon',
              'Tree2_Drop1A':   [],                 # 'Lime'
              },
# S3W2 Hidden AttackFX
'SBK_61':    {'HiddenRBlockA':  [],                 # 'AttackFXC'
              },
# S3E1
'SBK_64':    {'YBlockA':        [],                 # 'Coin'
              },
# Sarcophagus Hall 1
'ISK_02':    {'ItemA':          [],                 # 'SpikeShield'
              },
# Sand Drainage Room 1
'ISK_03':    {'ItemA':          [],                 # 'RuinsKey'
              },
# Pyramid Stone Room
'ISK_05':    {'ItemA':          [],                 # 'PyramidStone'
              },
# Sand Drainage Room 2
'ISK_06':    {'ItemA':          [],                 # 'StarPiece',
              'ItemB':          [],                 # 'RuinsKey'
              },
# Sarcophagus Hall 2
'ISK_07':    {'ItemA':          [],                 # 'RuinsKey',
              'ItemB':          [],                 # 'Artifact'
              },
# Super Hammer Room
'ISK_09':    {'ChestA':         [],                 # 'SlowGo'
              },
# Sand Drainage Room 3
'ISK_12':    {'ItemA':          [],                 # 'RuinsKey'
              },
# Lunar Stone Room
'ISK_13':    {'ItemA':          [],                 # 'LunarStone'
              },
# Diamond Stone Room
'ISK_14':    {'ItemA':          [],                 # 'DiamondStone'
              },
# Tree Face (Bub-ulb)
'MIM_04':    {'GiftA':          [],                 # 'MagicalSeed3'
              },
# Laughing Rock
'MIM_08':    {'RBlockA':        [],                 # 'HPPlusA'
              },
# Flowers Appear (FP Plus)
'MIM_09':    {'RBlockA':        [],                 # 'FPPlusA'
              },
# Outside Boo's Mansion
'MIM_11':    {'YBlockA':        [],                 # 'VoltShroom'
              #TODO StrangeLeaf in Bush ?? 
              },
# Pot Room
'OBK_05':    {'CrateA':         [],                 # 'Apple',
              'CrateB':         [],                 # 'Apple'
              },
# Library
'OBK_06':    {'ItemA':          [],                 # 'BoosPortrait',
              'CrateA':         [],                 # 'StarPiece'
              },
# Record Player Room
'OBK_07':    {'ChestA':         [],                 # 'Weight'
              },
# Wasteland Ascent 1
'ARN_02':    {'ItemA':          [],                 # 'DizzyDial',
              'ItemB':          [],                 # 'Letter07',
              'YBlockA':        [],                 # 'Coin',
              'YBlockB':        [],                 # 'Coin',
              'YBlockC':        [],                 # 'RepelGel'
              },
# Ghost Town 1
'ARN_03':    {'GiftA':          [],                 # 'Package',
              'YBlockA':        [],                 # 'Coin'
              },
# Wasteland Ascent 2
'ARN_04':    {'YBlockA':        [],                 # 'SuperShroom',
              'YBlockB':        [],                 # 'Coin',
              'ItemA':          [],                 # 'StarPiece'
              },
# Table/Clock Room (1/2F)
'DGB_03':    {'ItemA':          [],                 # 'StarPiece'
              },
# Basement
'DGB_06':    {'ChestA':         [],                 # 'CastleKey1'
              },
# Study (1F)
'DGB_07':    {'ItemA':          [],                 # 'StarPiece'
              },
# Covered Tables Room (1F)
'DGB_11':    {'ItemA':          [],                 # 'DDownJump'
              },
# Spike Trap Room (2F)
'DGB_12':    {'ChestA':         [],                 # 'CastleKey1'
              },
# Hidden Bedroom (2F)
'DGB_13':    {'ItemA':          [],                 # 'MegaRush',
              'ItemB':          [],                 # 'Coin',
              'ItemC':          [],                 # 'Coin',
              'ItemD':          [],                 # 'Coin',
              'ItemE':          [],                 # 'Coin',
              'ItemF':          [],                 # 'Coin',
              'ItemG':          [],                 # 'Coin'
              },
# Stairs to Third Floor
'DGB_14':    {'YBlockA':        [],                 # 'MapleSyrup'
              },
# Sleeping Clubbas Room (3F)
'DGB_16':    {'ItemA':          [],                 # 'CastleKey1'
              },
# BLU Large Playroom
'OMO_01':    {'ItemA':          [],                 # 'Calculator',
              'HiddenYBlockA':  [],                 # 'Mystery',
              'HiddenYBlockB':  [],                 # 'FrightJar'
              },
# RED Boss Barricade
'OMO_02':    {'YBlockA':        [],                 # 'SleepySheep',
              'HiddenYBlockA':  [],                 # 'Coin',
              'ItemA':          [],                 # 'ShootingStar'
              },
# BLU Block City
'OMO_04':    {'ItemA':          [],                 # 'Mushroom',
              'ChestA':         [],                 # 'StoreroomKey',
              'YBlockA':        [],                 # 'Coin',
              'YBlockB':        [],                 # 'Coin',
              'YBlockC':        [],                 # 'ThunderBolt',
              'ItemB':          [],                 # 'Coin',
              'ItemC':          [],                 # 'Coin',
              'ItemD':          [],                 # 'Coin',
              'ItemE':          [],                 # 'Coin',
              'ItemF':          [],                 # 'Coin',
              'ItemG':          [],                 # 'Coin',
              'ItemH':          [],                 # 'Coin',
              'ItemI':          [],                 # 'Coin',
              'ItemJ':          [],                 # 'StarPiece',
              'ItemK':          [],                 # 'StarPiece'
              },
# PNK Gourmet Guy Crossing
'OMO_05':    {'ItemA':          [],                 # 'Cookbook',
              'YBlockA':        [],                 # 'Coin',
              'YBlockB':        [],                 # 'Coin',
              'ItemB':          [],                 # 'Cookbook',
              'HiddenYBlockA':  [],                 # 'DizzyDial',
              'HiddenYBlockB':  [],                 # 'SuperSoda'
              },
# PNK Playhouse
'OMO_07':    {'ItemA':          [],                 # 'ThunderRage',
              'ChestA':         [],                 # 'FryingPan',
              'ChestB':         [],                 # 'DefendPlusA',
              'ChestC':         [],                 # 'IcePower',
              'YBlockA':        [],                 # 'Coin'
              },
# GRN Treadmills/Slot Machine
'OMO_09':   {'ItemA':           [],                 # 'SuperSoda',
             'ChestA':          [],                 # 'Dictionary',
             'ItemB':           [],                 # 'Coin',
             'ItemC':           [],                 # 'Coin',
             'ItemD':           [],                 # 'Coin',
             'ItemE':           [],                 # 'Coin',
             'ItemF':           [],                 # 'Coin',
             'ItemG':           [],                 # 'Coin',
             'ItemH':           [],                 # 'StarPiece',
             'ItemI':           [],                 # 'Coin',
             'ItemJ':           [],                 # 'Coin',
             'ItemK':           [],                 # 'Coin',
             'ItemL':           [],                 # 'Coin',
             'ItemM':           [],                 # 'Coin',
             'ItemN':           [],                 # 'Coin',
             'ItemO':           [],                 # 'MysteryNote'
             },
# RED Moving Platforms
'OMO_11':   {'HiddenYBlockA':   [],                 # 'VoltShroom',
             'HiddenYBlockB':   [],                 # 'SnowmanDoll',
             'YBlockA':         [],                 # 'Coin',
             'HiddenRBlockA':   [],                 # 'DeepFocus2',
             'YBlockB':         [],                 # 'Coin'
             },
# BLU Anti-Guy Hall
'OMO_13':   {'ChestA':          [],                 # 'PowerPlusB',
             'YBlockA':         [],                 # 'Coin',
             'HiddenYBlockA':   [],                 # 'MapleSyrup'
             },
# PNK Tracks Hallway
'OMO_17':   {'YBlockA':         [],                 # 'Coin',
             'YBlockB':         [],                 # 'Coin',
             'YBlockC':         [],                 # 'Coin'
             },
#Whale Cove
'JAN_00':   {'ItemA':           [],                 # 'Coin',
             'ItemB':           [],                 # 'Coin',
             'ItemC':           [],                 # 'StopWatch'
             },
# Beach
'JAN_01':   {'ItemA':           [],                 # 'Letter11',
             'HiddenYBlockA':   [],                 # 'RepelGel',
             'HiddenYBlockB':   [],                 # 'Mystery',
             'ItemB':           [],                 # 'Coin',
             'ItemC':           [],                 # 'Coin'
             },
# Sushi Tree
'JAN_04':   {'ChestA':          [],                 # 'VolcanoVase',
             'ItemA':           [],                 # 'StarPiece',
             'Tree2_Drop1A':    [],                 # 'Letter04'
             },
# SE Jungle (Quake Hammer)
'JAN_05':   {'RBlockA':         [],                 # 'PowerQuake'
             },
# NE Jungle (Raven Statue)
'JAN_06':   {'ItemA':           [],                 # 'Coin'
             },
# SW Jungle (Super Block)
'JAN_08':   {'ItemA':           [],                 # 'Coin',
             'ItemB':           [],                 # 'Coin',
             'ItemC':           [],                 # 'Coin',
             'HiddenYBlockA':   [],                 # 'ThunderRage'
             },
# Western Dead End
'JAN_10':   {'ItemA':           [],                 # 'StarPiece'
             },
# Deep Jungle 1
'JAN_12':   {'ItemA':           [],                 # 'Egg2',
             'HiddenYBlockA':   [],                 # 'StoneCap'
             },
# Deep Jungle 2 (Block Puzzle)
'JAN_13':   {'HiddenYBlockA':   [],                 # 'VoltShroom'
             },
# Deep Jungle 3
'JAN_14':   {'ItemA':           [],                 # 'FireFlower',
             'ItemB':           [],                 # 'Mushroom'
             },
# Great Tree Vine Ascent
'JAN_18':   {'ItemA':           [],                 # 'HappyHeartB'
             },
# Path to the Volcano
'JAN_22':   {'GiftA':           [],                 # 'UltraStone',
             'ItemA':           [],                 # 'JamminJelly'
             },
# Central Cavern
'KZN_03':   {'ItemA':           [],                 # 'FireShield',
             'ItemB':           [],                 # 'POWBlock',
             'YBlockA':         [],                 # 'Coin',
             'YBlockB':         [],                 # 'Coin',
             'YBlockC':         [],                 # 'Coin',
             'YBlockD':         [],                 # 'Coin'
             },
# Flowing Lava Puzzle
'KZN_06':   {'HiddenYBlockA':   [],                 # 'LifeShroom'
             },
# Dizzy Stomp Room
'KZN_08':   {'ChestA':          [],                 # 'DizzyStomp'
             },
# Boss Room
'KZN_19':   {'YBlockA':         [],                 # 'SuperShroom',
             'YBlockB':         [],                 # 'MapleSyrup'
             },
# (SW) Posie and Crystal Tree
'FLO_07':   {'ItemA':           [],                 # 'CrystalBerry',
             'ItemB':           [],                 # 'CrystalBerry',
             'GiftA':           [],                 # 'FertileSoil',
             'ItemC':           [],                 # 'CrystalBerry',
             'ItemD':           [],                 # 'CrystalBerry'
             }, #TODO
# (SE) Briar Platforming
'FLO_08':   {'ItemA':           [],                 # 'StarPiece'
             },
# (East) Triple Tree Path
'FLO_09':   {'ItemA':           [],                 # 'HappyFlowerB'
             },
# (SE) Lily's Fountain
'FLO_10':   {'GiftA':           [],                 # 'MiracleWater'
             }, #TODO Jammin Jelly ???
# (West) Rosie's Trellis
'FLO_12':   {'GiftA':           [],                 # 'WaterStone'
             },
# (NW) Lakilester
'FLO_13':   {'ItemA':           [],                 # 'MegaSmash',
             'ItemB':           [],                 # 'ShootingStar'
             },
# (NW) Bubble Flower
'FLO_14':   {'ItemA':           [],                 # 'StarPiece'
             },
# (NE) Elevators
'FLO_16':   {'ItemA':           [],                 # 'StarPiece'
             },
# (NE) Fallen Logs
'FLO_17':   {'HiddenYBlockA':   [],                 # 'ThunderRage',
             'ItemA':           [],                 # 'Letter09'
             },
# Cloudy Climb
'FLO_19':   {'ItemA':           [],                 # 'SJumpChg'
             },
# (East) Old Well
'FLO_22':   {'ItemA':           [],                 # 'FlowerSaverB',
             'ItemB':           [],                 # 'FlowerSaverB'
             }, #TODO

 'FLO_23': {'HiddenYBlockA': 'ShootingStar',
 'HiddenYBlockB': 'Coin'},

 'SAM_05': {'ItemA': 'Letter06',
 'HiddenYBlockA': 'StopWatch'},

 'SAM_06': {'GiftA': 'Scarf'},

 'SAM_07': {'HiddenYBlockA': 'UltraShroom'},

 'SAM_10': {'RBlockA': 'MegaJump',
 'ItemA': 'StarPiece'},

 'SAM_11': {'ItemA': 'WarehouseKey'},

 'SAM_12': {'ItemA': 'StarStone',
 'ItemB': 'StarStone'},

 'PRA_04': {'YBlockA': 'SuperShroom'},

 'PRA_05': {'ChestA': 'BlueKey'},

 'PRA_06': {'ItemA': 'ShootingStar'},

 'PRA_11': {'ChestA': 'RedKey'},

 'PRA_12': {'ChestA': 'PDownDUp'},

 'PRA_15': {'ItemA': 'StarPiece'},

 'PRA_27': {'ChestA': 'PalaceKey'},

 'PRA_28': {'ChestA': 'PUpDDown'},

 'PRA_35': {'ChestA': 'TripleDip'},

 'KPA_01': {'YBlockA': 'POWBlock'},

 'KPA_03': {'YBlockA': 'ShootingStar'},

 'KPA_10': {'YBlockA': 'LifeShroom'},

 'KPA_11': {'ItemA': 'CastleKey2',
 'YBlockA': 'LifeShroom'},

 'KPA_14': {'ItemA': 'Mystery',
 'ItemB': 'ThunderRage'},

 'KPA_15': {'ChestA': 'CastleKey2'},

 'KPA_17': {'CrateA': 'TastyTonic',
 'CrateB': 'LifeShroom'},

 'KPA_60': {'RBlockA': 'DeepFocus3'},

 'KPA_61': {'YBlockA': 'SuperShroom',
 'YBlockB': 'MapleSyrup',
 'YBlockC': 'SuperShroom',
 'ItemA': 'JamminJelly'},

 'KPA_62': {'RBlockA': 'DeepFocus3'},

 'KPA_91': {'ItemA': 'PrisonKey1'},

 'KPA_95': {'ItemA': 'PrisonKey1'},

 'KPA_100': {'ItemA': 'CastleKey2'},

 'KPA_101': {'ItemA': 'UltraShroom'},

 'KPA_111': {'YBlockA': 'SuperShroom',
 'HiddenYBlockA': 'MapleSyrup'},

 'KPA_119': {'ItemA': 'CastleKey2'},

 'KPA_133': {'ItemA': 'CastleKey2'},

 'KPA_134': {'HiddenYBlockA': 'MapleSyrup'},

 'OSR_02': {'HiddenYBlockA': 'UltraShroom'}}
